...
resource "aws_cloudfront_distribution" "projoy" {
  enabled             = true
  ...
  #we are saying no cache for all other except /image and /media
...
records = [
{
    ...
    type    = "A"
    alias   = {
    name    = aws_cloudfront_distribution.projoy.domain_name
    zone_id = aws_cloudfront_distribution.projoy.hosted_zone_id
    }
}
]
}